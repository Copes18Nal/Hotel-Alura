...
public class Busqueda extends JFrame {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if (filaReservas >= 0) {
					...
					if (confirmar == JOptionPane.YES_OPTION) {
						...
						reservaControlador.Eliminar(Integer.valueOf(valor));
					...
					if (confirmarh == JOptionPane.YES_OPTION) {
						...
						huespedesControlador.Eliminar(Integer.valueOf(valor));
						JOptionPane.showMessageDialog(contentPane, "Registro Eliminado");
						limpiarTabla();
						LlenarTablaHuespedes();
						LlenarTablaReservas();
	...
	private List<Reserva> MostrarReservas() {
		return this.reservaControlador.buscar();
	}
	...
	private void mostrarTablaReservas() {
		...
		try {
			for(Reserva reservas : reserva) {
				modelo.addRow(new Object[] {reservas.getId(), reservas.getDataE(), reservas.getDataS(), reservas.getValor(), 
	...
	private List<Reserva> BuscarReservasId() {
		return this.reservaControlador.buscarId(txtBuscar.getText());
	}

	private List<Huespedes> BuscarHuespedes() {
		return this.huespedesControlador.listarHuespedes();
	}

	private List<Huespedes> BuscarHuespedesId() {
		return this.huespedesControlador.listarHuespedesId(txtBuscar.getText());
	}
	...
	private void LlenarTablaReservas() {
		...
		// Llenar tabla
		List<Reserva> reserva = ActualizarReservas();
		try {
			for (Reserva reservas : reserva) {
	...
	private List<Reserva> ActualizarReservas() {
		Optional.ofNullable(modelo.getValueAt(tbReservas.getSelectedRow(), tbReservas.getSelectedColumn()))
				.ifPresentOrElse(fila -> {
					...
					this.reservaControlador.actualizar(fechaE, fechaS, valor, formaPago, id);
	...
	private void ActualizarHuesped() {
		Optional.ofNullable(modeloHuesped.getValueAt(tbHuespedes.getSelectedRow(), tbHuespedes.getSelectedColumn()))
				.ifPresentOrElse(filaHuesped -> {
					...
					this.huespedesControlador.actualizar(nombre, apellido, fechaN, nacionalidad, telefono, idReserva,
							id);
					JOptionPane.showMessageDialog(this, String.format("Registro modificado con éxito"));
				}, () -> JOptionPane.showMessageDialog(this, "Por favor, elije un registro"));
